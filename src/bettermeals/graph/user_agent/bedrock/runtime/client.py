...
Main client implementation for invoking the User Agent agent via Bedrock Runtime API.
...
class RuntimeAgentClient:
    ...
    async def invoke(
        ...
        try:
            ...
            logger.error(f"Error invoking user agent via Runtime: {str(e)}", exc_info=True)
            raise
